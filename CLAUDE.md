...
- Old palette: `.brandNavy` (~200 uses as of 2026-09-23 — being retired batch by batch; check new diffs for it) and `.brandGold`. The zero-use aliases (`.brandPrimary`, `.brandSecondary`, `.premium`, `.premiumBackground`) and the brand/button `LinearGradient` tokens (`brandNavy`, `brandGold`, `primaryButton`, `coachButton`, `premiumButton`, `premiumAccent`) were deleted 2026-09-23. `glassShine/Dark` survive only as the pre-iOS-26 fallback inside `GlassChrome.ppVideoOverlayPanel`; `glassBorder` is also the white hairline on the capture-overlay buttons.
...
- The ones still listed are **deprecated, not deleted** (only tokens with zero call sites get deleted) — they still compile and resolve, so nothing fails loudly when someone reaches for one. Treat any doc claiming the palette migration "finished" as stale; verify with a symbol-name grep.
