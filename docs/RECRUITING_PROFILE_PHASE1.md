...
### Schema (V35)
> Originally built as V33 on the feature branch; renumbered to **V35** at merge time because main had since consumed V33 (drills/season types/headshots) and V34 (pitching stats).
- **`PlayerPathSchema.swift`** — `enum SchemaV35` (mirrors V34) + appended to `schemas` + a `.lightweight(V34→V35)` stage.
- **`PlayerPathApp.swift`** — both container inits bumped to `Schema(SchemaV35.models)`.
- ⚠️ The runtime container has **no `migrationPlan:`** by design (all `SchemaV#` enums reference the same live classes → a real plan throws duplicate-checksum). The load-bearing change is **adding the optional property to the live `Athlete` class**; SwiftData does automatic lightweight migration. The `SchemaV35` enum + stage are convention bookkeeping only.
...
- ⬜ On-device over-the-top **V34→V35 migration test** (a failed migration silently falls back to in-memory and *looks* like data loss).
