...
final class GamesViewModel: ObservableObject {
    ...
    func create(opponent: String, date: Date, isLive: Bool, season: Season? = nil, golfDetails: GolfRoundDetails? = nil, location: String? = nil, tournament: GolfTournament? = nil, allowDuplicate: Bool = false, onDuplicate: @escaping () -> Void = {}, onError: @escaping (String) -> Void) {
        ...
        // so unassigned games would silently move sports when the user
        ...
        Task {
            let result = await gameService.createGame(for: athlete, opponent: opponent, date: date, isLive: isLive, season: resolvedSeason, allowDuplicate: allowDuplicate, golfDetails: golfDetails, location: location, tournament: tournament)
            switch result {
            case .success:
                break
            case .failure(.duplicateGame):
                // Same opponent, same day — a possible doubleheader. Let the view
                // confirm before re-creating with allowDuplicate. Never fires for
                // golf (exempt) or when allowDuplicate is already true.
                onDuplicate()
