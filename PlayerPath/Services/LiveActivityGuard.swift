...
/// confirmation surfaced before starting a new round / practice round /
...
enum LiveActivityGuard {
    ...
    /// The athlete's live golf round (a golf `Game`), if any.
    ...
    /// True when a golf round OR practice is currently live.
    static func hasAnyLiveGolf(for athlete: Athlete) -> Bool {
        liveGolfGame(for: athlete) != nil || liveGolfPractice(for: athlete) != nil
    }

    /// Short noun for the confirmation dialog body, e.g. "round",
    /// "practice round", "range session". Nil when nothing is live.
    static func currentLiveGolfLabel(for athlete: Athlete) -> String? {
        // A golf Game is a ROUND — "tournament" is the multi-round container.
        if let game = liveGolfGame(for: athlete) { return game.eventNounLowercased }
