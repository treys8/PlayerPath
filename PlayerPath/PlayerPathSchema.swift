...
// V34: pitching box-score fields (outsRecorded/earnedRuns/runsAllowed/hitsAllowed/
// homeRunsAllowed/battersFaced) added to AthleteStatistics + GameStatistics for the
// full pitching stat suite (ERA/WHIP/IP) and manual pitching entry. Lightweight —
// new defaulted Int fields only.
enum SchemaV34: VersionedSchema {
...
// MARK: - Schema V35 (2026-07-15 — Recruiting profile)
//
//  Changes from V34:
//    • Athlete.recruitingProfileJSON (String? = nil) — JSON-encoded RecruitingInfo
//      bio blob backing the recruiting profile (one synced field, not ~20 columns).
//
//  A single new optional column → lightweight migration is sufficient; existing
//  rows read back recruitingProfileJSON == nil. No new model, no relationship change.
enum SchemaV35: VersionedSchema {
...
// MARK: - Schema V37 (2026-08-03 — Multi-coach share links)
//
//  Changes from V36:
//    • VideoClip.sharedCoachVideoIDs ([String] = []) — every shared-folder copy this
//      clip has been shared to. V36's single `sharedCoachVideoID` was overwritten by
//      a second share, which silently stopped the FIRST coach's feedback from
//      resolving in the Journal feed and the player (it survived only in the folder
//      browser). The scalar is retained as "most recent share" for the player's
//      default target and for compatibility with older builds and already-written
//      Firestore docs.
//
//  A single new column with a non-nil default → lightweight migration is sufficient;
//  existing rows read back an empty array, and `allCoachFeedbackVideoIDs` unions the
//  scalar so they keep resolving. No new model, no relationship change.
enum SchemaV37: VersionedSchema {
